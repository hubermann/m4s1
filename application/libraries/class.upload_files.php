...
*/
        ...
        {
         $full = ImageJPEG( $full_id, $this->user_full_name,90);
        }
    ...
    function start_copy()
    {
        ...
        if (!isset($this->error))
        {
            ...
            if (!@move_uploaded_file ($this->user_tmp_name,$destination))
             $this->error = "Impossible to copy ".$this->user_file_name." from $user_file to destination directory.";
        }
    }
...
*/
        ...
        if(preg_match("/\.(jpg|jpeg)/",$this->thumb_name))
        {
         $full = ImageJPEG( $full_id, $this->thumb_name,90);
        }
